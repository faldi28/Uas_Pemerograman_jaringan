...
const RegisterUser = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      await API.post("/register", { username, password, role: "user" });
      navigate("/login-user");
    } catch (err) {
  ...
  return (
    <div className="container is-max-desktop p-4">
      <div className="box p-6">
        <h2 className="title is-5 has-text-centered mb-5">Register User</h2>
        {error && (
        ...
        <form onSubmit={handleRegister}>
          ...
          <div className="field mb-4">
            <div className="control">
              <button className="button is-primary is-fullwidth">Register User</button>
            </div>
          </div>
        </form>
      </div>
